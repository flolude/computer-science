...
\begin{itemize}
	\item \texttt{FindMin(A,i)}
	      ...
	      \end{algorithmic}
	      \texttt{FindMin} braucht höchstens $\floor{\frac{n+1}{2}}$ Schritte und hat somit eine Laufzeit von $\text{O}(n)$.
\end{itemize}  
...
\[
	...
	5 \cdot \frac{n}{7} \text{log }\frac{n}{7} \leq c \cdot n\text{ log }n;
\]
\[ \implies c \geq \frac{5}{7} \cdot 1- \text{log}_n 7;\quad  n>1 \]
\[ \implies T(n) \in \Theta(f(n))=\Theta(n \text{ log }n) \]
\vspace*{1\baselineskip}
...
\[ \text{Fall 1, weil } f(n) \in \text{O} (n^{\frac{1}{2} - \epsilon}) \]
\[ \implies T(n) \in \Theta (n^\frac{1}{2}) \]
\vspace*{1\baselineskip}
...
\[
	a\cdot f(\frac{n}{b}) \leq c \cdot f(n) \Leftrightarrow
	16 \cdot {(\frac{n}{4})}^2 \text{ log } \frac{n}{4} \leq c \cdot n^2\text{ log }n
\]
\[ \implies c\ge 1-2 \cdot \text{log} _n 2; \quad n>1 \]
\[ \text{Es gibt kein n für welches die Regularitätsbedingung gilt.} \]
\[ \text{Das Mastertheorem kann also nicht angewendet werden!} \]
\end{document}